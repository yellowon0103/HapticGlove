...
#elif Vive
            // 테스트 수정
            Transform controller = ARAVRInput.LHand.GetChild(0);
            // 컨트롤러의 forward 방향 벡터를 반환합니다.
...
# endif
    ...
    public static bool Get(Button virtualMask, Controller hand = Controller.LTouch)
    {
#if PC
    ...
    public static bool GetDown(Button virtualMask, Controller hand = Controller.LTouch)
    {
#if PC
    ...
    public static bool GetUp(Button virtualMask, Controller hand = Controller.LTouch)
    {
#if PC
    ...
    public static float GetAxis(string axis, Controller hand = Controller.RTouch)
    {
#if PC
    ...
    public static void DrawCrosshair(Transform crosshair, bool isHand = true, Controller hand = Controller.LTouch)
    {
        Ray ray;
        ...
        */
        ...
        if (plane.Raycast(ray, out distance))
        {
            ...
            crosshair.position = hitPosition;
            crosshair.forward = -Camera.main.transform.forward;
            // 크로스헤어의 크기를 최소 기본 크기에서 거리에 따라 더 커지도록 한다.
            crosshair.localScale = originScale * Mathf.Max(1, distance);
        }
